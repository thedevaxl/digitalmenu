...
const daysOfWeek: IWorkingHour[] = [
  ...
  { day: 'Thursday', morningOpen: '09:30', morningClose: '13:30', afternoonOpen: '15:30', afternoonClose: '19:30', closed: false },
  { day: 'Friday', morningOpen: '09:30', morningClose: '13:30', afternoonOpen: '15:30', afternoonClose: '19:30', closed: false },
...
const Admin = () => {
  ...
  const handleWorkingHoursChange = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value, type, checked } = e.target;
    const newWorkingHours = [...form.workingHours];
    newWorkingHours[index] = { ...newWorkingHours[index], [name]: type === 'checkbox' ? checked : value };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const endpoint = form.id ? `/api/restaurant?id=${form.id}` : '/api/restaurant';
  ...
  const handleMenuCategoryChange = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
  ...
  const handleAddDish = (menuIndex: number) => {
    const newMenu = [...form.menu];
    newMenu[menuIndex].dishes.push({ name: '', price: 0, ingredients: [], allergens: [] });
    setForm({ ...form, menu: newMenu });
  };
  ...
  const handleDishChange = (menuIndex: number, dishIndex: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    const newMenu = [...form.menu];
    newMenu[menuIndex].dishes[dishIndex] = { ...newMenu[menuIndex].dishes[dishIndex], [name]: value };
    setForm({ ...form, menu: newMenu });
  };

  const handleIngredientChange = (menuIndex: number, dishIndex: number, ingredientIndex: number, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleRemoveIngredient = (menuIndex: number, dishIndex: number, ingredientIndex: number) => {
    const newMenu = [...form.menu];
    newMenu[menuIndex].dishes[dishIndex].ingredients.splice(ingredientIndex, 1);
    setForm({ ...form, menu: newMenu });
  };

  const handleAllergenChange = (menuIndex: number, dishIndex: number, allergenIndex: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const { value } = e.target;
    const newMenu = [...form.menu];
  ...
  const handleRemoveAllergen = (menuIndex: number, dishIndex: number, allergenIndex: number) => {
    const newMenu = [...form.menu];
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="form-control">
          ...
          {form.workingHours.map((wh, index) => (
            <div key={index} className={`space-y-2 ${wh.closed ? 'text-gray-500' : ''}`}>
        ...
        <div className="form-control">
                ...
                <div key={dishIndex} className="space-y-2 border p-2">
                  ...
                  <div className="space-y-1">
                    ...
                    {dish.ingredients.map((ingredient, ingIndex) => (
                      <div key={ingIndex} className="flex space-x-2 items-center">
                        <input
                          type="text"
                          name="ingredient"
                          ...
                          value={ingredient}
                          onChange={(e) => handleIngredientChange(index, dishIndex, ingIndex, e)}
                        />
                        <button
                          type="button"
                          className="btn btn-danger"
                          onClick={() => handleRemoveIngredient(index, dishIndex, ingIndex)}
                        >
                          Remove
                        </button>
                      </div>
                  ...
                  <div className="space-y-1">
                    ...
                    {dish.allergens.map((allergen, allergenIndex) => (
                      <div key={allergenIndex} className="flex space-x-2 items-center">
                        <input
                          type="text"
                          name="allergen"
                          ...
                          className="input input-bordered flex-1"
                          value={allergen}
                          onChange={(e) => handleAllergenChange(index, dishIndex, allergenIndex, e)}
                        />
                        <button
                          type="button"
                          className="btn btn-danger"
                          onClick={() => handleRemoveAllergen(index, dishIndex, allergenIndex)}
                        >
                          Remove
                        </button>
                      </div>
          ...
          <button type="button" className="btn btn-secondary mt-2" onClick={handleAddMenuCategory}>
            Add Menu Category
          </button>
        </div>
              ...
              <li key={restaurant._id.toString()}  className="border p-4 rounded">
                ...
                <div>
                  ...
                  {restaurant.workingHours.map((wh, index) => (
                    <div key={index} className={`${wh.closed ? 'line-through' : ''}`}>
                      <p>{wh.day}</p>
                      {!wh.closed && (
                        <>
                          <p>Morning: {wh.morningOpen} - {wh.morningClose}</p>
                          <p>Afternoon: {wh.afternoonOpen} - {wh.afternoonClose}</p>
                        </>
                      )}
